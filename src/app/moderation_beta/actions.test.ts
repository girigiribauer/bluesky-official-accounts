...
describe("approveEntry", () => {
  it("未ログインのとき、エラーを返す", async () => {
    ...
    const result = await approveEntry("entry-1", "account-1");
...
describe("rejectEntry", () => {
  it("未ログインのとき、エラーを返す", async () => {
    mockGetCurrentModerator.mockResolvedValueOnce(null);
    const result = await rejectEntry("entry-1", "account-1", "理由");
    expect(result).toEqual({ ok: false, error: "ログインが必要です" });
  });

  it("正常なとき、entries を rejected に更新して activity を記録する", async () => {
    const result = await rejectEntry("entry-1", "account-1", "");
    expect(result).toEqual({ ok: true });
    expect(mockFrom).toHaveBeenCalledWith("entries");
    expect(mockFrom).toHaveBeenCalledWith("activities");
  });
  ...
  it("却下理由があるとき、evidences にも記録する", async () => {
    const result = await rejectEntry("entry-1", "account-1", "根拠不十分");
    expect(result).toEqual({ ok: true });
    expect(mockFrom).toHaveBeenCalledWith("evidences");
  });

  it("却下理由が空白のみのとき、evidences に記録しない", async () => {
    await rejectEntry("entry-1", "account-1", "   ");
...
describe("updateEntryTwitterHandle", () => {
  it("未ログインのとき、エラーを返す", async () => {
    mockGetCurrentModerator.mockResolvedValueOnce(null);
    const result = await updateEntryTwitterHandle("entry-1", "account-1", "newhandle");
    expect(result).toEqual({ ok: false, error: "ログインが必要です" });
  });
  ...
  it("ハンドルの前後の空白をトリムして保存する", async () => {
    await updateEntryTwitterHandle("entry-1", "account-1", "  newhandle  ");
...
describe("updateEntryBlueskyHandle", () => {
  it("未ログインのとき、エラーを返す", async () => {
    mockGetCurrentModerator.mockResolvedValueOnce(null);
    const result = await updateEntryBlueskyHandle("entry-1", "account-1", "new.bsky.social");
    expect(result).toEqual({ ok: false, error: "ログインが必要です" });
  });
  ...
  it("ハンドルが空のとき、エラーを返す", async () => {
    const result = await updateEntryBlueskyHandle("entry-1", "account-1", "");
...
describe("updateEntryStatus", () => {
  it("未ログインのとき、エラーを返す", async () => {
    mockGetCurrentModerator.mockResolvedValueOnce(null);
    const result = await updateEntryStatus("entry-1", "account-1", "migrated");
    expect(result).toEqual({ ok: false, error: "ログインが必要です" });
  });
  ...
  it("ステータスが空のとき、エラーを返す", async () => {
    const result = await updateEntryStatus("entry-1", "account-1", "");
...
describe("setEntryClassification", () => {
  it("未ログインのとき、エラーを返す", async () => {
    mockGetCurrentModerator.mockResolvedValueOnce(null);
    const result = await setEntryClassification("account-1", "a1b2c3d4-e5f6-4a7b-8c9d-000000000001");
    expect(result).toEqual({ ok: false, error: "ログインが必要です" });
  });

  it("正常なとき、account_fields の classification_id を更新する", async () => {
    const result = await setEntryClassification("account-1", "a1b2c3d4-e5f6-4a7b-8c9d-000000000001");
    expect(result).toEqual({ ok: true });
  ...
  it("classification_id が空のとき、null として保存する", async () => {
    await setEntryClassification("account-1", "");
    expect(mockUpdate).toHaveBeenCalledWith(
