...
async function getPendingEntrySubmissions(): Promise<ReviewSubmission[]> {
  ...
  const submissions = (data ?? []) as unknown as (ReviewSubmission & { existing_bluesky_handle: null })[];

  const dids = submissions.map((s) => s.bluesky_did).filter(Boolean);
  if (dids.length === 0) return submissions;
  ...
  const existingMap = new Map((existingEntries ?? []).map((e) => [e.bluesky_did, e.bluesky_handle]));

  return submissions.map((s) => ({
    ...s,
    existing_bluesky_handle: existingMap.get(s.bluesky_did) ?? null,
  }));
}
...
export default async function DashboardLayout({
  ...
  if (!moderator.is_admin) {
    const { data: myMemberships } = await getSupabaseClient()
      .from("field_memberships")
      .select("field_id")
      .eq("moderator_id", moderator.id);

    if (!myMemberships || myMemberships.length === 0) {
      redirect("/moderation_beta/onboard");
    }
  }
